...
variable "prod_vpc_cidr" {
  ...
  default     = "10.0.0.0/16"
    validation {
        condition     = can(regex("^(10|192\\.168|172\\.(1[6-9]|2[0-9]|3[0-1]))\\.(\\d{1,3})\\.(\\d{1,3})/(16|20|24)$", var.prod_vpc_cidr))
...
variable "dev_vpc_cidr" {
  ...
  default     = "10.1.0.0/16"
    validation {
        condition     = can(regex("^(10|192\\.168|172\\.(1[6-9]|2[0-9]|3[0-1]))\\.(\\d{1,3})\\.(\\d{1,3})/(16|20|24)$", var.dev_vpc_cidr))
        error_message = "The CIDR block must be a valid private IP range."
    }
}
